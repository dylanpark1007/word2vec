...
if __name__ == '__main__':
    ...
    for i in range(100,len(allfiles)):
        if i == 100 :
            train_first(fi=allfiles[i], fo='file', cbow=False, neg=15, dim=300, alpha=0.025, win=5, min_count=5, num_processes=4,
              binary=False,subsamp =0)
        else :
            train(fi=allfiles[i], fo='file', cbow=False, neg=15, dim=300, alpha=0.025, win=5, min_count=5, num_processes=4,
                  binary=False,subsamp =0)
            print(i,'번째 진행 완료')
