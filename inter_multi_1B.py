...
class Vocab:
    def __init__(self, fi, min_count):
        ...
        self.re_vocab_hash = dict(zip(self.vocab_hash.values(), self.vocab_hash.keys()))
...
def test_analogy(norm_y1, y2, analogy_data):
    ...
    for num, target_token in enumerate(target_list):
        pred_list = {}
        for i, vector in enumerate(norm_y1):
            cosine_value = cosine(target_token, vector)
            pred_list[i] = math.sqrt(cosine_value**2)
...
def init_net(dim, vocab_size):
    ...
    syn0 = np.random.uniform(low=-0.5, high=0.5, size=(vocab_size, dim))  # 사이즈가 V X N이고 각 값이 -0.5/N 에서 +0.5/N사이의 값들을 random uniform하게 테이블로 생성
...
def train_process(pid): #pid는 range(num_processes)
    ...
    analogy_data = test_dataset('analogy task.txt', vocab_list)
    ...
    while epoch_count < num_epoch :
        ...
        while fi.tell() < end:
            ...
            for sent_pos, token in enumerate(sent):  # sent의 단어와 인덱스
                ...
                context = sent[context_start:sent_pos] + sent[
                                                         sent_pos + 1:context_end]  # Turn into an iterator? context는 중심단어를 제외한 앞뒤 context word list
                ...
                if word_count % 1000000 == 0 :
                    ...
                    print(test_analogy(norm_y, vocab_list, analogy_data),'%')
...
if __name__ == '__main__':
    ...
    train(fi='text8', fo='file', cbow = False, neg=60, dim=64, alpha=0.001, win=5, min_count=5,num_processes=1, binary=False, subsamp =1)
...
corpus = []
count = 0
with open('./data/1-billion-word-language-modeling-benchmark-r13output/heldout-monolingual.tokenized.shuffled/news.en.heldout-00000-of-00050','r',encoding='utf-8') as f:
    line = f.readlines()
    while line == '':
        corpus.append(line)
        line = f.readlines()
