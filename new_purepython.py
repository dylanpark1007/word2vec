...
class UnigramTable:
    ...
    def __init__(self, vocab):
        ...
        table_size = np.uint32(1e7)  # Length of the unigram table
...
def __init_process(*args):
    ...
    vocab, syn0, syn1, table, cbow, neg, dim, starting_alpha, win, num_processes, global_word_count, subsamp = args[:-1]
    fi = open(args[-1], 'r')
...
if __name__ == '__main__':
    ...
    for i in range(100,len(allfiles)):
        if i == 100 :
            train_first(fi=allfiles[i], fo='file', cbow=False, neg=0, dim=300, alpha=0.025, win=5, min_count=5, num_processes=1,
              binary=False,subsamp =1)
        else :
            train(fi=allfiles[i], fo='file', cbow=False, neg=0, dim=300, alpha=0.025, win=5, min_count=5, num_processes=1,
                  binary=False,subsamp =1)
            print(i,'번째 진행 완료')
